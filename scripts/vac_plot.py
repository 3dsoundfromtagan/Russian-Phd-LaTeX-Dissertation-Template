...
file = pathlib.Path(r'../raw data/3564/2/from power  LOT OF TIMES/'
                    + r'202410031417.csv')
...
plt.style.use(r'../conf67/conf.mplstyle')
...
ax.plot(up['power']/10, up['green'] / up['power'] * 100 / 1000 * podgon_sh_power, '.',
        markersize=3, label='Power increase')
ax.plot(down['power']/10, down['green'] / down['power'] * 100 / 1000 * podgon_sh_power, '.',
        markersize=3, label='Power decrease')
ax.set_xlabel('Pump peak power, kW')
ax.set_ylabel('SHG efficiency, %')
ax.axvline(x=0.05, color='#B62C0C', linestyle='--',
           linewidth=2, label='PM-curves measurements level')
